$(document).ready(function(){
    $("#input-form").submit(function(event){
     ...
     // console.log(inputcombined)
      ...
      var translateToPigLatin = function(inputPerWords, inputPerChar) {
      ...
      var result = translateToPigLatin(inputPerWords, inputPerChar);
  
      var special = /^[a-zA-Z0-9]/gi;

      console.log(result);
      // code below does not yet work to remove special characters
      var resultSplit = result.join(" ").split("");
      console.log(resultSplit);
      for (var y = 0; y < result.length; y++){
        if (result[y].match(special)) {
          console.log(result[y]);
          
          console.log(resultSplit);
          for (var x = 0; x < resultSplit.length; x++) {
            if (resultSplit[x].match(special)) {
              resultSplit[x].slice();
            }
          }
        } else {
